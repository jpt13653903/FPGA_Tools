...
// As a side-note: CHS addressing assumes:
// - 63 sectors per head
// - 128 heads per cylinder
// - Sectors count from 1, heads and cylinders count from 0
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
        ...
        5'h10: begin
          if(ResponseCRC && CardPresent) begin // Valid response
            ...
            end else if((BlockAddress < BLOCK_NR) && BlockRead) begin
              TimeOut      <= 26'd39_999_999; // 40e6 => 1 sec
            ...
            end else if((BlockAddress < BLOCK_NR) && BlockWrite) begin
              TimeOut      <= 26'd39_999_999; // 40e6 => 1 sec
